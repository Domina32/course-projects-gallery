# -*- coding: utf-8 -*-
"""
Created on Wed Oct 21 17:28:04 2020
...
subkeys = ["243f6a88", "85a308d3", "13198a2e", "03707344", "a4093822", 
                   "299f31d0", "082efa98", "ec4e6c89", "452821e6", "38d01377", 
                   "be5466cf", "34e90c6c", "c0ac29b7", "c97c50dd", "3f84d5b5", 
                   "b5470917", "9216d5d9", "8979fb1b"]

def add_mod(a,b): #ulaz: hex, izlaz: dec
    suma = 0
    a, b = int(str(a),16), int(str(b),16)
    suma = (a + b) % 4294967296

    return suma

def hex_to_bin(hexa): #rijeseno s ugradenim funkcijama pythona
    binary = ''
    for i in range(len(hexa)):
        num = int(hexa[i] + '', 16)
        bin4 = bin(num)[2:]
        bin4 = '0000'+bin4
        bin4 = bin4[len(bin4)-4:]
        binary += bin4
    #print(binary)
    return binary


def f(open_text): #ulaz: hex, izlaz: hex
    a = []
    for i in range(0,8,2):
        col = int((bin(int(open_text[i:i+2], 16))[2:]),2)
        print(bin(int(open_text[i:i+2], 16))[2:])
        print(col)
        a.append(S_boxes[i//2][col])
    
    ans = hex(add_mod(a[0], a[1]))[2:]
    ans = hex(int(ans,16) ^ int(a[2],16))[2:]
    ans = hex(add_mod(ans, a[3]))[2:]
...
def gen_key(key):
    ...
    for i in range(len(subkeys)):
        subkeys[i] = hex(int(subkeys[i],16) ^ int(key[j:j+8],16))[2:]
        j = (j + 8) % len(key)
        

def runda(time, otvoreni_tekst): #ulaz: int time, hex tekst, izlaz: hex
    left = otvoreni_tekst[:8]
    right = otvoreni_tekst[8:16]
    left = hex(int(left,16) ^ int(subkeys[time],16))[2:]
    
    Foutput = f(left)
    
    right = hex(int(Foutput,16) ^ int(right,16))[2:]
    
    return right + left


def encrypt(otvoreni_tekst): #ulaz: hex, izlaz: hex
    for i in range(16):
        #print('runda: ',i+1,' daje: ', otvoreni_tekst)
        otvoreni_tekst = runda(i, otvoreni_tekst)
        
    #postprocesiranje
    right = otvoreni_tekst[:8]
    left = otvoreni_tekst[8:16]
    
    right = hex(int(right,16) ^ int(subkeys[16],16))[2:]
    left = hex(int(left,16) ^ int(subkeys[17],16))[2:]
...
def decrypt(sifrat):
    for i in range(17, 1, -1):
        print(sifrat)
        sifrat = runda(i, sifrat)
        
    #postprocesiranje
    right = sifrat[:8]
    left = sifrat[8:16]
    
    right = hex(int(right,16) ^ int(subkeys[1],16))[2:]
    left = hex(int(left,16) ^ int(subkeys[0],16))[2:]
    
    return left + right
        

def go():
    otvoreni_tekst = "123456abcd132536"
    key = "aabb09182736ccdd"
    
    gen_key(key)
    
    cipher = encrypt(otvoreni_tekst)
...
def go_d():
    sifrat = "d748ec383d3405f7"
    key = "aabb09182736ccdd"
    
    gen_key(key)
    
    otvoreni_tekst = decrypt(sifrat)
    
    print('šifrat: ', sifrat)
    print('~ dešifriranje ~')
    print('otvoreni tekst: ', otvoreni_tekst)
    
    #sljedece 2 funkcije su mi sluzile za provjere
def cipher(otvoreni):
    key = "aabb09182736ccdd"
    gen_key(key)
    
    ciph = encrypt(otvoreni)
