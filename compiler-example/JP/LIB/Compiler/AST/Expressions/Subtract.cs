...
{
    class Subtract : BinarOp
    {
        public Subtract(Expression left, Expression right) //prenese 2 podcvora - lijevi i desni operand
            : this(left, right, null)
        { }

        public Subtract(Expression left, Expression right, Coordinate coord)
        ...
        public override bool Interpret(IdnTable idnTable, Memmory memmory, List<ErrorBase> errors)
        {
            ...
            {
                errors.Add(new ErrorBase(ErrorTypes.Semantic, string.Format("Oduzimanje tipova {0} i {1} nije moguće", this.left.DType.ToString(), this.right.ToString())));
                return false;
            }
