...
{
    public class Mul : BinarOp
    {
        public Mul(Expression left, Expression right)
            :this(left, right, null)
        { }

        public Mul(Expression left, Expression right, Coordinate coord)
        ...
        public override bool Interpret(IdnTable idnTable, Memmory memmory, List<ErrorBase> errors)
        {
            ...
            {
                errors.Add(new ErrorBase(ErrorTypes.Semantic, string.Format("Množenje tipova {0} i {1} nije moguće", this.left.DType.ToString(), this.right.ToString())));
                return false;
            }
