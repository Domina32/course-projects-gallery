...
{
    public class LogicalOr : BinarOp
    {
        public LogicalOr(Expression left, Expression right) //prenese 2 podcvora - lijevi i desni operand
            : this(left, right, null)
        { }

        public LogicalOr(Expression left, Expression right, Coordinate coord)
        ...
        public override bool Interpret(IdnTable idnTable, Memmory memmory, List<ErrorBase> errors)
        {
            ...
            {
                errors.Add(new ErrorBase(ErrorTypes.Semantic, string.Format("Or tipova {0} i {1} nije moguće", this.left.DType.ToString(), this.right.ToString())));
                return false;
            }
        ...
        {
            return string.Format("({0}o{1})", this.left.ToString(), this.right.ToString());
        }
    }
}
