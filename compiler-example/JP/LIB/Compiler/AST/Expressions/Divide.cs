...
namespace LIB.Compiler.AST.Expressions
{
    class Divide : BinarOp
    {
        ...
        public Divide(Expression left, Expression right, Coordinate coord)
        ...
        public override bool Interpret(IdnTable idnTable, Memmory memmory, List<ErrorBase> errors)
        {
            ...
            {
                errors.Add(new ErrorBase(ErrorTypes.Semantic, string.Format("Dijeljenje tipova {0} i {1} nije moguće", this.left.DType.ToString(), this.right.ToString())));
                return false;
            }
